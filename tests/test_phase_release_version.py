...
def test_phase_7_release_uses_roadmap_version():
    ...
    assert '__version__ = "0.7.0"' in main_source
    assert "Enkryon-v0.7.0.apk" in readme
    assert (
        "versions use `major.phase.subphase` as the roadmap reference"
        in normalized_guide
    )
    assert (
        "Phase 7 completion therefore prepares `v0.7.0` from "
        "`v0.6.0`."
        in normalized_guide
    )
